...
if (isset($_POST['add_book'])) {
    ...
    if (!empty($_POST['new_category'])) {
        ...
        $conn->query("INSERT INTO Category (Category_Name) 
                      SELECT '$category' WHERE NOT EXISTS 
                      (SELECT 1 FROM Category WHERE Category_Name='$category')");
    } else {
        $category = $_POST['category'];
    }
    ...
    if (!empty($_POST['new_location'])) {
        ...
        $conn->query("INSERT INTO Location (Location_Name) 
                      SELECT '$location' WHERE NOT EXISTS 
                      (SELECT 1 FROM Location WHERE Location_Name='$location')");
    } else {
        $location = $_POST['location'];
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".category-wrapper, .location-wrapper").forEach(wrapper => {
    ...
    // Hide input field if clicked outside and empty
    document.addEventListener("click", e => {
      if (!e.target.closest("." + wrapper.className)) {
