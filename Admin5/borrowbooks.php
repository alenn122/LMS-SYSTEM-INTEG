...
<button type="button" 
    ...
    onclick="addBook('<?php echo $book['Book_ID']; ?>', '<?php echo htmlspecialchars($book['Title']); ?>', <?php echo $book['Available_Copies']; ?>)">
    Add
</button>
...
// Add book
// Add book with availability check
window.addBook = function(id, title, available){
    ...
    item.innerHTML = `<span><strong>${title}</strong></span>
...
confirmYesBtn.addEventListener("click", function(){
    fetch('saveBorrow.php',{
        ...
        if(data.success){
            ...
            loadBooks(); // refreshes availability after successful borrow


            // ---- NEW CODE TO UPDATE AVAILABLE ----
            borrowedBooks.forEach(bookId => {
                const bookCard = document.querySelector(`.book-card button[onclick*="'${bookId}'"]`);
                if(bookCard){
                    let available = parseInt(bookCard.dataset.available) || 0;
                    available--; // decrease available
                    if(available <= 0){
                        bookCard.textContent = "No available";
                        bookCard.disabled = true;
                        bookCard.classList.remove("btn-primary");
                        bookCard.classList.add("btn-secondary");

                    }
                    bookCard.dataset.available = available; // update dataset
                }
            });
...
if(book.Available_Copies > 0){
    actionHTML = `<button class="btn btn-sm btn-primary" onclick="addBook('${book.Book_ID}','${book.Title}', ${book.Available_Copies})">Add</button>`;
} else {
    actionHTML = `<button class="btn btn-sm btn-secondary" disabled>No Available</button>`;
}
        ...
        if(query.length > 0){
            ...
            .then(data=>{
                ...
                if(data.length > 0){
                    ...
                    suggestions.innerHTML = "<div class='list-group-item text-muted'>No Available</div>";
                    suggestions.style.display = "block";
                }
            });
        } else {
...
// --- Function to perform the search and update bookResults ---
function performSearch(query) {
    if (!query) {
        loadBooks(); // load default list if empty
        suggestions.style.display = "none";
        return;
    }
